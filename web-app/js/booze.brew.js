...
/**
 * BoozeBrew class containing all methods for brew process handling
 * 
 */
function BoozeBrew() {

	/**
	 * Indicates if the last update call is already finished
	 * 
	...
	/**
	 * Actually ommited updates count
	 * 
	 * @type Integer
	 */
	this.updateOmits = 0;
	...
	/**
	 * Unique process id to identify ourselves to the server
	 * 
	 * @type String
	 */
	this.processId = "";
...
BoozeBrew.prototype.init = function(processId, options) {
	...
	if (options.updateTimeout)
		this.updateTimeout = options.updateTimeout;
	if (options.coolingStep)
		this.coolingStep = true;

	this.processId = processId;

	this.dialogs.initDialog = booze.notifier.confirm($('#initDialogTemplate')
			.tmpl({}), {
...
BoozeBrew.prototype.resumeLostSession = function(processId, options) {
	...
	this.lauterTemperatureReached({}, "ficken");
};
...
BoozeBrew.prototype.startBrewing = function() {
	...
	$.get(APPLICATION_ROOT + '/brew/start', {
		...
		if (data.success) {
			booze.notifier.removeDialog(booze.brew.dialogs.initDialog)
...
BoozeBrew.prototype.ajaxError = function(jqxhr) {
	...
	booze.notifier.error(booze.messageSource
			.message("js.booze.brew.serverCommunicationError"));
...
/**
 * Inits an AJAX call to the server to update the current brew process status
 * 
...
BoozeBrew.prototype.update = function() {
	...
	booze.brew.updateTimeoutHandle = setTimeout(booze.brew.update,
			booze.brew.updateTimeout);
	...
	if (booze.brew.updateStatus == 0) {
		if (booze.brew.updateOmits < booze.brew.maxUpdateOmits) {
			...
			booze.log.info("Omitted " + booze.brew.maxUpdateOmits
					+ " updates, proceeding");
			if (!booze.brew.dialogs.stalled) {
				booze.brew.dialogs.stalled = booze.notifier
						.error(
								booze.messageSource
										.message("js.booze.brew.serverCommunicationStalled"),
								{
									callback : function() {
										booze.notifier
												.removeDialog(booze.brew.dialogs.stalled);
									}
								});
			}
	...
	$.get(APPLICATION_ROOT + '/brew/readStatus', {
		processId : booze.brew.processId
	}).success(booze.brew.updateCallback).error(booze.brew.updateError);
};

/**
 * Callback for update server calls Updates all fields reported by the server
 * 
...
BoozeBrew.prototype.updateCallback = function(response) {
	...
	if (response.success === false) {
		...
		booze.notifier.error(response.error, {
			...
			buttonText : booze.messageSource.message('js.booze.brew.hijackSession'),
			modal : true
		})
		return;
	}
	...
	try {
		...
		for (i = 0; i < data.temperatureSensors.length; i++) {
			...
			$(sensor).find('.temperatureSensorValue').first().html(
					data.temperatureSensors[i].temperature);
			$(sensor)
					.find('.progressbar')
					.first()
					.progressbar(
							{
								value : Math
										.round(data.temperatureSensors[i].temperature / 1.1)
							});
			...
			// TODO: use calculateTemperatureColor for progress bar color
			// modification
		}

		for (i = 0; i < data.pressureSensors.length; i++) {
			sensor = $('#pressureSensor_' + data.pressureSensors[i].id);
			$(sensor).find('.pressureSensorValue').first().html(
					data.pressureSensors[i].pressure);
		...
		for (i = 0; i < data.heaters.length; i++) {
			if (data.heaters[i].forced == true) {
				...
				if (data.heaters[i].power != undefined) {
					if ($('#heater_' + data.heaters[i].id + '_forced').find(
							".slider").first().hasClass("sliding") === false) {
						$('#heater_' + data.heaters[i].id + '_forced').find(
								".slider").first().slider("value",
								data.heaters[i].power);
					}
				}
				...
				if (data.heaters[i].enabled === true) {
					$('#heater_' + data.heaters[i].id + '_forced').find(
							'.statusIcon').first().addClass("ui-state-active");
				} else {
					$('#heater_' + data.heaters[i].id + '_forced').find(
							'.statusIcon').first().removeClass(
							"ui-state-active");
				}
			} else {
				...
				if (data.heaters[i].power != undefined) {
					if (data.heaters[i].enabled === true) {
						$('#heater_' + data.heaters[i].id + '_progressbar')
								.find('.progressbar').first().progressbar(
										"value", data.heaters[i].power);
					} else {
						$('#heater_' + data.heaters[i].id + '_progressbar')
								.find('.progressbar').first().progressbar(
										"value", 0);
				...
				// Set status icon (on/off)
				if (data.heaters[i].enabled === true) {
					$('#heater_' + data.heaters[i].id + '_regular').find(
							'.statusIcon').first().addClass("ui-state-active");
				} else {
					$('#heater_' + data.heaters[i].id + '_regular').find(
							'.statusIcon').first().removeClass(
		...
		for (i = 0; i < data.motors.length; i++) {
			if (data.motors[i].forced == true) {
				...
				if (data.motors[i].speed != undefined) {
					$('#motor_' + data.motors[i].id + '_forced').find(
							'.indicator').first().html(data.motors[i].speed);
				}
				...
				if (data.motors[i].speed != undefined) {
					if (data.motors[i].enabled === true) {
						$('#motor_' + data.motors[i].id + '_progressbar').find(
								'.progressbar').first().progressbar("value",
								data.motors[i].speed);
					} else {
						$('#motor_' + data.motors[i].id + '_progressbar').find(
								'.progressbar').first().progressbar("value", 0);
					}
				}

				// Set status icon (on/off)
				if (data.motors[i].enabled === true) {
					$('#motor_' + data.motors[i].id + '_regular').find(
							'.statusIcon').first().addClass("ui-state-active");
				} else {
					$('#motor_' + data.motors[i].id + '_regular').find(
							'.statusIcon').first().removeClass(
	...
	if ($(booze.brew.dialogs.updateErrorDialog).dialog("isOpen") === true) {
		booze.notifier.removeDialog(booze.brew.dialogs.updateErrorDialog);
	}
	if ($(booze.brew.dialogs.stalled).dialog("isOpen") === true) {
		booze.notifier.removeDialog(booze.brew.dialogs.stalled);
	}
};
...
BoozeBrew.prototype.updateError = function(jqxhr) {
	...
	booze.brew.dialogs.updateErrorDialog = booze.notifier
			.error(
					booze.messageSource
							.message("js.booze.brew.serverCommunicationError"),
					{
						callback : function() {
							booze.notifier
									.removeDialog(booze.brew.dialogs.updateErrorDialog);
						}
					});
...
BoozeBrew.prototype.cancel = function(options) {
	...
	if (options.force) {
		...
		booze.brew.dialogs.cancelDialog = booze.notifier
				.confirm(booze.messageSource
						.message("js.booze.brew.cancelConfirmation"), {
					proceedCallback : booze.brew.cancel,
					proceedCallbackOptions : {
						force : true
					},
					proceedText : booze.messageSource
							.message("js.booze.notifier.confirmCancellation"),
					cancelText : booze.messageSource
							.message("js.booze.notifier.keepOnBrewing"),
					cancelCallback : function() {
						booze.notifier
								.removeDialog(booze.brew.dialogs.cancelDialog);
					}
				});
	}
};
...
BoozeBrew.prototype.handleEvent = function(event) {
	...
	if (event.playSound) {
		$.sound.play(APPLICATION_ROOT + "/sounds/notification.mp3", {
			timeout : 10000
		});
	}
...
BoozeBrew.prototype.openDialog = function(dialog, args, message) {
	/**
	 * Possible dialogs - addHop - cookingFinished - mashingTemperatureReached -
	 * lauterTemperatureReached - mashingElongationFinished - pressureExceeded
	 */
	...
	try {
		...
		booze.notifier.error(booze.messageSource
				.message("js.booze.brew.couldNotOpenDialog")
				+ dialog);
...
BoozeBrew.prototype.pressureExceeded = function(args, message) {
	if ($(booze.brew.dialogs.pressureExceededDialog).dialog("isOpen") === true) {
		booze.notifier.removeDialog(booze.brew.dialogs.pressureExceededDialog);
	}
...
BoozeBrew.prototype.addHop = function(args, message) {
	...
	booze.notifier.notify($('#addHopDialogTemplate').tmpl(args),
			{
				title : booze.messageSource
						.message('js.booze.brew.addHopDialog.title'),
			});
};
...
BoozeBrew.prototype.lauterTemperatureReached = function(args, message) {
	...
	booze.brew.dialogs.lauterTemperatureReachedDialog = booze.notifier
		.notify(
				$('#lauterTemperatureReachedDialogTemplate').tmpl(args),
				{
					title : booze.messageSource
							.message('js.booze.brew.lauterTemperatureReachedDialog.title'),
					callback : booze.brew.startCooking,
					buttonText: booze.messageSource.message('js.booze.brew.startCooking')
				});
...
BoozeBrew.prototype.cookingFinished = function(args, message) {
	...
	// Display brew initialization window
	booze.brew.dialogs.cookingFinishedDialog = booze.notifier.notify($(
			'#cookingFinishedDialogTemplate').tmpl(args), {
		title : booze.messageSource
				.message('js.booze.brew.cookingFinishedDialog.title'),
		buttonText : booze.messageSource
				.message('js.booze.brew.cookingFinishedDialog.finish'),
		callback : cb
	});
};
...
BoozeBrew.prototype.mashingTemperatureReached = function(args, message) {
	...
	booze.brew.dialogs.mashingTemperatureReachedDialog = booze.notifier
			.notify(
					$('#mashingTemperatureReachedDialogTemplate').tmpl(args),
					{
						title : booze.messageSource
								.message('js.booze.brew.mashingTemperatureReachedDialog.title'),
			...
			.success(
					function(data) {
						if (data.success === true) {
							...
							booze.notifier
									.removeDialog(booze.brew.dialogs.mashingTemperatureReachedDialog);
						} else {
							booze.notifier.error(data.message);
						}
	...
	$
			...
			.success(
					function(data) {
						if (data.success === true) {
							booze.notifier
									.removeDialog(booze.brew.dialogs.cookingFinishedDialog);

							if (booze.brew.dialogs.coolingDialog) {
								booze.notifier
										.removeDialog(booze.brew.dialogs.coolingDialog);
							}

							booze.brew.dialogs.coolingDialog = booze.notifier
									.notify(
											$('#coolingDialogTemplate')
													.tmpl({}),
											{
												title : booze.messageSource
														.message('js.booze.brew.coolingDialog.title'),
												callback : booze.brew.finish,
												buttonText : booze.messageSource
														.message('js.booze.brew.finishBrewProcess')
											})
						} else {
							booze.notifier.error(data.message);
						}
...
BoozeBrew.prototype.startCooking = function() {
	...
	$.get(APPLICATION_ROOT + '/brew/startCooking', {
		processId : booze.brew.processId,
		finalSpargingWaterVolume : $(
				'#brewLauterTemperatureReachedDialog_finalSpargingWaterVolume')
				.val(),
		finalPostSpargingWort : $(
				'#brewLauterTemperatureReachedDialog_finalPostSpargingWort')
				.val(),
		finalPreSpargingWort : $(
				'#brewLauterTemperatureReachedDialog_finalPreSpargingWort')
				.val()
	})
	.success(
		function(data) {
			if (data.success === true) {
				...
				booze.notifier
						.removeDialog(booze.brew.dialogs.lauterTemperatureReachedDialog);
			} else {
				booze.notifier.error(data.message);
			}
...
BoozeBrew.prototype.elongateMashing = function() {
	...
	$
			.get(
					APPLICATION_ROOT + '/brew/elongateMashing',
					{
						processId : booze.brew.processId,
						time : $(
								'#brewLauterTemperatureReachedDialog_elongateMashingTime')
								.val()
					})
			.success(
					function(data) {
...
BoozeBrew.prototype.elongateCooking = function() {
	...
	$
			.get(
					APPLICATION_ROOT + '/brew/elongateCooking',
					{
						processId : booze.brew.processId,
						time : $(
								'#brewCookingTemperatureReachedDialog_elongateCookingTime')
								.val(),
						temperature : $('#cookingTemperatureSlider').slider(
								"value")
					})
			.success(
					function(data) {
...
BoozeBrew.prototype.handleStep = function(step) {
	...
	case "BrewTargetMashingTemperatureStep":
		...
		for ( var i = 0; i < children.length; i++) {
			if (children[i].id != "targetMashingTemperatureInfo")
				$(children[i]).hide();
			else
				$(children[i]).show();
		}
		$('#targetMashingTemperatureInfo_targetTemperature').html(
				step.targetTemperature);
		$('#targetMashingTemperatureInfo_stepStartTime').html(
				step.stepStartTime);
		break;
	...
	case "BrewMashingTemperatureReachedStep":
		var children = $('#stepInfo').children();
		for ( var i = 0; i < children.length; i++) {
			if (children[i].id != "mashingTemperatureReachedInfo")
				$(children[i]).hide();
			else
				$(children[i]).show();
		}
	...
	case "BrewRestStep":
		...
		if (step.targetTemperatureReached) {
			$('#restInfo_temperatureReached_true').html(
					step.targetTemperatureReachedTime);
	...
	case "BrewTargetLauterTemperatureStep":
		var children = $('#stepInfo').children();
		for ( var i = 0; i < children.length; i++) {
			if (children[i].id != "mashingFinishedInfo")
				$(children[i]).hide();
			else
				$(children[i]).show();
		}
		$('#mashingFinishedInfo_targetTemperature')
				.html(step.targetTemperature);
	...
	case "BrewInitCookingStep":
		var children = $('#stepInfo').children();
		for ( var i = 0; i < children.length; i++) {
			if (children[i].id != "initCookingInfo")
				$(children[i]).hide();
			else
				$(children[i]).show();
		}
	...
	case "BrewCookingStep":
		...
		if (step.targetTemperatureReached) {
			$('#cookingInfo_temperatureReached_true').html(
					step.targetTemperatureReachedTime);
	...
	case "BrewElongateMashingStep":
		var children = $('#stepInfo').children();
		for ( var i = 0; i < children.length; i++) {
			if (children[i].id != "elongateMashingInfo")
				$(children[i]).hide();
			else
				$(children[i]).show();
		}
		...
		$('#elongateMashingInfo_targetTemperature')
				.html(step.targetTemperature);
	...
	case "BrewElongateCookingStep":
		...
		for ( var i = 0; i < children.length; i++) {
			if (children[i].id != "cookingInfo"
					&& children[i].id != "elongateCookingInfo")
				$(children[i]).hide();
			else
				$(children[i]).show();
		}
		...
		$('#elongateCookingInfo_targetTemperature')
				.html(step.targetTemperature);
...
BoozeBrew.prototype.setCookingTemperature = function(value) {
	booze.log.info("Setting cooking temperature to " + value);
...
BoozeBrew.prototype.toggleForceHeater = function(heater) {
	$.get(APPLICATION_ROOT + '/brew/toggleForceHeater', {
		...
		heater : heater
			...
			function(data) {
				if (data.success === true) {
					$("#heater_" + heater + "_forced").find(".slider").first()
							.slider(
									"value",
									$('#heater_' + heater + '_progressbar')
											.find('.progressbar').first()
											.progressbar("value"));
 ...
 * 
 */
BoozeBrew.prototype.setForcedHeaterPower = function(heater, power) {
	$.get(APPLICATION_ROOT + '/brew/setForcedHeaterPower', {
		processId : booze.brew.processId,
		heater : heater,
		power : power
	}).success(
			function(data) {
				if (data.success === true) {
					booze.log.info("set heater power for heater " + heater
							+ " to: " + power);
				} else {
					booze.notifier.error(data.message);
				}
...
BoozeBrew.prototype.toggleForcedHeaterStatus = function(heater, status) {
	$.get(APPLICATION_ROOT + '/brew/toggleForcedHeaterStatus', {
		processId : booze.brew.processId,
		heater : heater
	}).success(function(data) {
		if (data.success === true) {
...
BoozeBrew.prototype.spinAliveThrobber = function() {
	...
	$('#aliveThrobber').attr(
			'src',
			APPLICATION_ROOT + "/images/icons/brew/throbber/"
					+ this.aliveThrobber + ".png");
};
...
BoozeBrew.prototype.showCalculator = function() {
	...
	if ($(booze.brew.showCalculatorDialog).dialog("isOpen") === true) {
		booze.log.info("calculator window already open, moving it to top");
		$(booze.brew.showCalculatorDialog).dialog("moveToTop");
		return;
	}

	// Display brew initialization dialog
	booze.brew.showCalculatorDialog = booze.notifier.notify($(
			'#brewCalculatorDialogTemplate').tmpl({}), {
...
BoozeBrew.prototype.editProtocolData = function() {
	if ($(booze.brew.dialogs.editProtocolData).dialog("isOpen") === true) {
		booze.log.info("protocol data window already open, moving it to top");
	...
	$
			...
			.success(
					function(data) {
						if (data.success === true) {
							booze.brew.dialogs.editProtocolData = booze.notifier
									.confirm(
											data.html,
											{
												title : booze.messageSource
														.message("js.booze.brew.editProtocolDialog.title"),
												proceedCallback : booze.brew.saveProtocolData,
												proceedText : booze.messageSource
														.message("js.booze.brew.save"),
												cancelCallback : function() {
													booze.notifier
															.removeDialog(booze.brew.dialogs.editProtocolData);
												}
											});
						} else {
							booze.notifier.error(data.message);
...
BoozeBrew.prototype.saveProtocolData = function() {
	$
			.get(
					APPLICATION_ROOT + '/brew/saveProtocolData',
					$('#protocolDataForm').serialize() + "&processId="
							+ booze.brew.processId)
			.success(
					function(data) {
						if (data.success === true) {
							if (data.close == true) {
...
BoozeBrew.prototype.showTemperatureChart = function() {
	...
	if ($(booze.brew.dialogs.temperatureChart).dialog("isOpen") === true) {
		booze.log
				.info("temperature chart window already open, moving it to top");
		$(booze.brew.dialogs.temperatureChart).dialog("moveToTop");
		return;
	}

	var now = new Date();
	booze.brew.dialogs.temperatureChart = $(
			'<div><img src="' + $('#temperatureChartDialogTemplate').html()
					+ '&q=' + now.getTime() + '" style="width: 100%" /></div>')
			.dialog(
					{
						width : "50%",
						minWidth : "35em",
						...
						close : function() {
							booze.notifier
									.removeDialog(booze.brew.dialogs.temperatureChart);
						},
						title : booze.messageSource
								.message("js.booze.brew.temperatureChart"),
						modal : false,
						buttons : []
					});
}
...
BoozeBrew.prototype.showPressureChart = function() {
	...
	if ($(booze.brew.dialogs.pressureChart).dialog("isOpen") === true) {
		booze.log.info("pressure chart window already open, moving it to top");
		$(booze.brew.dialogs.pressureChart).dialog("moveToTop");
		return;
	}

	var now = new Date();
	booze.brew.dialogs.pressureChart = $(
			'<div><img src="' + $('#pressureChartDialogTemplate').html()
					+ '&q=' + now.getTime() + '" style="width: 100%" /></div>')
			.dialog(
					{
						width : "50%",
						...
						close : function() {
							booze.notifier
									.removeDialog(booze.brew.dialogs.pressureChart);
						},
						title : booze.messageSource
								.message("js.booze.brew.pressureChart"),
						modal : false,
						buttons : []
					});
}
 ...
 * @type String
 */
BoozeBrew.prototype.calculateTemperatureColor = function(actualTemperature,
		targetTemperature) {
