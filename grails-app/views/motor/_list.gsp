...
<ul>
  ...
  <g:each in="${it.motors}" var="motor">
    <li>
      <div class="name" onclick="booze.setting.editDevice('motor', {'motor.id':'${motor.id}', 'setting.id':'${it.id}'})"><g:fieldValue bean="${motor}" field="name" /></div> <div class="deleteButton" onclick="booze.setting.deleteDevice('motor', {'motor.id':'${motor.id}', 'setting.id':'${it.id}'}); return false;"><span class="booze-icon booze-icon-delete"></span></div>
  ...
  <li class="pagination">
    <div class="createLink" onclick="booze.setting.editDevice('motor', {'setting.id':'${it.id}'});"><span class="left booze-icon booze-icon-add"></span> <span class="createText">Motor hinzufügen</span></div>
  </li>
</ul>